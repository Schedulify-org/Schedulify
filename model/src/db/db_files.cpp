...
bool DatabaseFileManager::updateFile(int fileId, const string& fileName, const string& fileType) {
    if (!db.isOpen()) {
        Logger::get().logError("Database not open for file update");
        return false;
    }

    if (fileId <= 0) {
        Logger::get().logError("Invalid file ID for update: " + std::to_string(fileId));
        return false;
    }

    if (fileName.empty()) {
        Logger::get().logError("Cannot update file with empty name");
        return false;
    }
    ...
    QSqlQuery query(db);
    query.prepare(R"(
        UPDATE file SET file_name = ?, file_type = ?, updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
    )");
    ...
    if (!query.exec()) {
        Logger::get().logError("Failed to update file ID " + std::to_string(fileId) + ": " + query.lastError().text().toStdString());
        return false;
    }

    int rowsAffected = query.numRowsAffected();
    if (rowsAffected == 0) {
        Logger::get().logWarning("No file found with ID " + std::to_string(fileId) + " for update");
        return false;
    }

    Logger::get().logInfo("File updated successfully: " + fileName + " (ID: " + std::to_string(fileId) + ")");
    return true;
}
...
int DatabaseFileManager::getFileCount() {
    if (!db.isOpen()) {
        Logger::get().logError("Database not open for file count");
        return -1;
    }

    QSqlQuery query("SELECT COUNT(*) FROM file", db);

    if (!query.exec()) {
        Logger::get().logError("Failed to get file count: " + query.lastError().text().toStdString());
        return -1;
    }
    ...
    Logger::get().logError("Unexpected error getting file count");
    return -1;
}

QDateTime DatabaseFileManager::getFileUploadTime(int fileId) {
    if (!db.isOpen()) {
        Logger::get().logError("Database not open for upload time retrieval");
        return QDateTime();
    }

    if (fileId <= 0) {
        Logger::get().logError("Invalid file ID for upload time: " + std::to_string(fileId));
        return QDateTime();
    }

    QSqlQuery query(db);
    query.prepare("SELECT upload_time FROM file WHERE id = ?");
    query.addBindValue(fileId);

    if (!query.exec()) {
        Logger::get().logError("Failed to get upload time for file ID " + std::to_string(fileId) + ": " + query.lastError().text().toStdString());
        return QDateTime();
    }
    ...
    if (query.next()) {
        ...
        if (!uploadTime.isValid()) {
            Logger::get().logWarning("Invalid upload time found for file ID: " + std::to_string(fileId));
        }
        return uploadTime;
    }

    Logger::get().logWarning("No upload time found for file ID: " + std::to_string(fileId));
    return QDateTime();
}

vector<FileEntity> DatabaseFileManager::getFilesByType(const string& fileType) {
    vector<FileEntity> files;

    if (!db.isOpen()) {
        Logger::get().logError("Database not open for files by type retrieval");
        return files;
    }

    if (fileType.empty()) {
        Logger::get().logError("Cannot get files with empty type");
        return files;
    }
    ...
    if (!query.exec()) {
        Logger::get().logError("Failed to retrieve files by type '" + fileType + "': " + query.lastError().text().toStdString());
        return files;
    }
    ...
    while (query.next()) {
        try {
            ...
            Logger::get().logError("Error parsing file data for type '" + fileType + "': " + string(e.what()));
            continue;
        }
    }

    Logger::get().logInfo("Retrieved " + std::to_string(files.size()) + " files of type: " + fileType);
    return files;
}

vector<FileEntity> DatabaseFileManager::getRecentFiles(int limit) {
    vector<FileEntity> files;

    if (!db.isOpen()) {
        Logger::get().logError("Database not open for recent files retrieval");
        return files;
    }
    ...
    if (!query.exec()) {
        Logger::get().logError("Failed to retrieve recent files: " + query.lastError().text().toStdString());
        return files;
    }
    ...
    while (query.next()) {
        try {
            ...
            Logger::get().logError("Error parsing recent file data: " + string(e.what()));
            continue;
        }
    }
